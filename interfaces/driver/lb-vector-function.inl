...
void VectorFunction::call(Result &res, const std::pair<const VectorKey, VectorAbstract*> &vector, const Functor &functor){
...
void VectorFunction::call(const std::pair<const VectorKey, VectorAbstract*> &v, const Functor &f){
  void *dumbresult;
  call(dumbresult,v,f);
}
...
template<class Functor, class Result>
void VectorFunction::call(Result &res, const VectorKey &key, const Functor &functor){
...
template<class Functor>
void VectorFunction::call(const VectorKey &k, const Functor &f) {
  void *dumbresult;
  call(dumbresult,k,f);
}
