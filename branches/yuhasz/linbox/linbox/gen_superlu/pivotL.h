 ...
 */
  ...
  if ( jcol == MIN_COL ) {
    ...
    for (k = nsupc; k < nsupr; k++) 
      ...
      cout << "  lu["<< lsub_ptr[k] << "] "<< lu_col_ptr[k] << "\n";
  }
#endif
  ...
  for (isub = nsupc; isub < nsupr; ++isub) {
    F.assign(rtemp, lu_col_ptr[isub]);
    if (!F.isZero(rtemp)) {
   ...
   *pivrow = lsub_ptr[pivptr]; // try with it back in //  A. Duran excluded 12.23.2002
   perm_r[*pivrow] = jcol;     // try with it back in //  A. Duran excluded 12.23.2002


#ifdef DEBUGPERMR
    cout<< "At pivotL .. test for singularity.. jcol "<< jcol <<"\n";
    cout<< "pivrow "<<*pivrow<<"\n";
    cout << "lsub_ptr[pivptr] : " << lsub_ptr[pivptr] << "\n" ;
    cout << "jcol : "<< jcol <<"\n";
    cout<< "pivmax "<<pivmax<<"\n"; 
    cout<< "pivptr "<<pivptr<<"\n";
#endif
...
#ifndef UNPARAMETRICFIELDFD // A. Duran added
    F.assign(rtemp, lu_col_ptr[old_pivptr]);
    if ( !F.isZero(rtemp) ) // ?
      pivptr = old_pivptr;
    else
      *usepr = 0;
...
#ifndef UNPARAMETRICFIELDFD // A. Duran added
      F.assign(rtemp, lu_col_ptr[diag]);
      if ( !F.isZero(rtemp) ) pivptr = diag; //?
#else
