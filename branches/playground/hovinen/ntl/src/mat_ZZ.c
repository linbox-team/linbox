...
static
void ll_solve1(ZZ& d_out, vec_ZZ& x_out,
           const mat_ZZ& A, const vec_ZZ& b, long force_hensel)
{
   long n = A.NumRows();
   ...
   if (!force_hensel && 
        NumBits(num_bound) + NumBits(den_bound) > 0.80*double(n)*double(n)) {
      // CRT-based method should be faster 
      ...
      // now have to fix up denominators to have consistent output
   ...
   if (max_A_len + NTL_NBITS + NumBits(n) <= NTL_DOUBLE_PRECISION-1)
      use_double_mul1 = 1;

   if (!use_double_mul1 && max_A_len+NTL_NBITS+2 <= NTL_DOUBLE_PRECISION-1) {
      use_double_mul2 = 1;
      double_limit = (1L << (NTL_DOUBLE_PRECISION-1-max_A_len-NTL_NBITS));
   }
   ...
   if (max_A_len + NTL_NBITS + NumBits(n) <= NTL_BITS_PER_LONG-1)
      use_long_mul1 = 1;

   if (!use_long_mul1 && max_A_len+NTL_NBITS+2 <= NTL_BITS_PER_LONG-1) {
      use_long_mul2 = 1;
      long_limit = (1L << (NTL_BITS_PER_LONG-1-max_A_len-NTL_NBITS));
   }
   ...
   if (use_double_mul1 || use_double_mul2) {
      double_h = new double[n];
      double_A = new double_ptr[n];
      if (!double_h || !double_A) Error("solve1: out of mem");

      for (i = 0; i < n; i++) {
         double_A[i] = new double[n];
