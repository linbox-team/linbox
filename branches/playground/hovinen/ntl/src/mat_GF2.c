...
{  
   ...
   long i;  
   for (i = 0; i < n; i++) {
      u_long *xp = X[i].rep.elts();
      const u_long *ap = A[i].rep.elts();
      const u_long *bp = B[i].rep.elts();
      long j;
      for (j = 0; j < mw; j++)
         xp[j] = ap[j] ^ bp[j];
   }
...
void det(GF2& d, const mat_GF2& M_in)
{
   long k, n;
   long i, j;
   long pos;
   ...
   for (k = 0; k < n; k++) {
      ...
      if (pos != -1) {
         ...
         u_long *y = M[k].rep.elts();

         for (i = k+1; i < n; i++) {
            // M[i] = M[i] + M[k]*M[i,k]

            if (M[i].rep.elts()[wk] & k_mask) {
               u_long *x = M[i].rep.elts();
