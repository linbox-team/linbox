...
{   protected:
	...
	{ Elt w;   return mulin(x, _nat.image(w, a)); }

	/// z <-- a*x + y
	Elt& saxpy(Elt& z, const SrcElt& a, const Elt& x, const Elt& y)
	{ Elt w;   return axpy(z, _nat.image(w, a), x, y); }

	/// y <-- a*x + y
	Elt& saxpyin(Elt& y, const SrcElt& a, const Elt& x)
	{ Elt w;   return axpyin(y, _nat.image(w, a), x); }
...
//
template<> uint32& ImageField<Modular<uint16>, Modular<uint32> >::
smul(uint32& y, const uint16& a, const uint32 & x)
	{ return mul(y, static_cast<uint32>(a), x); }
