...
static bool testMulAdd (const Field& F, size_t n, int iterations)
{
	...
	for (int k=0;k<iterations; ++k) {
		...
		MD.add(T,A,B);
		OMD.muladd(R,malpha,D,alpha,T,C);
...
bool CheckMulAdd( const Integer & alpha , const BlasMatrix<Integer> & A , const BlasMatrix<Integer> & B , const Integer & beta , const BlasMatrix<Integer> & C)
{
	...
	OpenCLMatrixDomain<ModularField> OMD (Zp);
	...
	if (!pass) {
		...
		std::cout << "#########################################" << std::endl;
#endif
		mycommentator.report() << " *** OMD ERROR (" << alpha << ',' << beta << ") *** " << std::endl;
	}
...
static bool testMulAddShapeTrans (const Field &F, size_t m, size_t n, size_t k, int iterations)
{
	...
	// A,B
	OMD.muladd(E,beta,C,alpha,A,B);
	if (!MD.areEqual(E,D)) {
		ret = false ;
		mycommentator.report() << " *** OMD ERROR (" << alpha << ',' << beta << ") (noTrans, noTrans) *** " << std::endl;
	}

	OMD.muladd(E,beta,C,alpha,A,tB);
	if (!MD.areEqual(E,D))  {
		ret = false ;
		mycommentator.report() << " *** OMD ERROR (" << alpha << ',' << beta << ") (noTrans, Trans) *** " << std::endl;
	}

	OMD.muladd(E,beta,C,alpha,tA,B);
	if (!MD.areEqual(E,D)) {
		ret = false ;
		mycommentator.report() << " *** OMD ERROR (" << alpha << ',' << beta << ") (Trans, noTrans) *** " << std::endl;
	}

	OMD.muladd(E,beta,C,alpha,tA,tB);
	if (!MD.areEqual(E,D)) {
		ret = false ;
		mycommentator.report() << " *** OMD ERROR (" << alpha << ',' << beta << ") (Trans, Trans) *** " << std::endl;
	}
...
static bool testTriangMulShapeTrans (const Field &F, size_t m, size_t n, int iterations)
{
	...
	if (LeftSide)
		OMD.muladd(D,one,B,one,A,B);
	else
		OMD.muladd(D,one,B,one,B,A);
	...
	/****  DIRECT ****/
	{
		...
		if(LeftSide) {
			OMD.mulin_right(L,E) ; // B <- AB
			OMD.mulin_right(U,G) ;
		}
		else {
			OMD.mulin_left(G,L) ;  // B <- BA
			OMD.mulin_left(E,U) ;
		}
		OMD.addin(E,G);
		...
		if (!MD.areEqual(E,D)) {
			ret = false ;
			mycommentator.report() << " *** OMD ERROR (" << (LeftSide?"left":"right") << ',' << (UnitDiag?" L":" U") << " is unit) *** " << std::endl;
		}
		else {
	...
	/****  Transpose ****/
	{
		...
		if (!MD.areEqual(E,D)) {
			ret = false ;
			mycommentator.report() << " *** OMD ERROR Transpose (" << (LeftSide?"left":"right") << ',' << (UnitDiag?" L":" U") << " is unit) *** " << std::endl;
		}
		else {
	...
	for (int k=0;k<iterations; ++k) {
		...
		for (size_t i=0;i<r;++i){
			...
			for (size_t j=i+1;j<n;++j)
                ...
                OMD.write(commentator.report(), S) << std::endl;
		...
		for (size_t i=0;i<n;++i){
			...
			L.setEntry(i,i,Gn.random(tmp));
                ...
                OMD.write(commentator.report(), L) << std::endl;

		//  compute A=LS
		OMD.mul(A,L,S);
                OMD.write(commentator.report(), A) << std::endl;

		// compute the rank of A
		unsigned int rank= OMD.rankin(A);
...
static bool testTriangularSolve (const Field& F, size_t m, size_t n, int iterations)
{
	...
	for (int k=0;k<iterations;++k) {
		...
		OMD.left_solve(X,TAl,B);
		OMD.mul(C,Al,X);
		if (!MD.areEqual(C,B))
			ret=false;

		OMD.left_solve(X,TAu,B);
		OMD.mul(C,Au,X);
		if (!MD.areEqual(C,B))
			ret=false;

		// testing solver with matrix left hand side
		OMD.right_solve(X,TAl,B);
		OMD.mul(C,X,Al);
		if (!MD.areEqual(C,B))
			ret=false;

		OMD.right_solve(X,TAu,B);
		OMD.mul(C,X,Au);
		if (!MD.areEqual(C,B))
			ret=false;
...
static bool testPermutation (const Field& F, size_t m, int iterations)
{
	...
	for (int k=0;k<iterations;++k) {
		...
		OMD.mul( B, A, Perm);
		// C = B.P^t
		OMD.mul( C, B, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm) );
		// Test C==A
		if (!MD.areEqual(A,C))
			ret=false;
		/*
		...
		// B = A.P^t
		OMD.mul( B, A, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm));
		// C = B.P
		OMD.mul( C, B, Perm );
		// Test C==A
		if (!MD.areEqual(A,C))
		...
		OMD.mul( B, Perm, A);
		// C = P^t.B
		OMD.mul( C, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm) , B);
		// Test C==A
		if (!MD.areEqual(A,C))
			ret=false;
		/*
		...
		// B = P^t.A
		OMD.mul( B, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm), A);
		// C = P.B
		OMD.mul( C, Perm, B);
		// Test C==A
		if (!MD.areEqual(A,C))
		...
		OMD.mul( Abis, Perm, A);
		// C = (P.A)^-1.B
		OMD.left_solve( C, Abis, B);
		// D = A.C (= P^-1.B)
		OMD.mul(D, A, C);
		// D = P.D
		OMD.mulin_right( Perm,D);
		if (!MD.areEqual(D,B))
			ret=false;
		/*
		...
		OMD.mul( Abis, Perm, A);
		// C = B.(P.A)^-1
		OMD.right_solve( C, Abis, B);
		// C = C.P
		OMD.mulin_left( C,Perm);
		// D = C.A (=B)
		OMD.mul(D, C, A);
		if (!MD.areEqual(D,B))
		  ret=false;
		...
		// Abis = P^t.A
		OMD.mul( Abis, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm), A);
		// C = (P^t.A)^-1.B
		OMD.left_solve( C, Abis, B);
		// D = A.C (= P.B)
		OMD.mul(D, A, C);
		// D = P^t.D
		OMD.mulin_right( TransposedBlasMatrix<BlasPermutation<size_t> >(Perm),D);
		if (!MD.areEqual(D,B))
			ret=false;
		/*
		...
		// Abis = A.P^t
		OMD.mul( Abis, A, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm));
		// C = (A.P^t)^-1.B
		OMD.left_solve( C, Abis, B);
		// C = P^t.C
		OMD.mulin_right( TransposedBlasMatrix<BlasPermutation<size_t> >(Perm),C);
		// D = A.C (= P.B)
		OMD.mul(D, A, C);
		...
		OMD.right_solve( C, Abis, B);
		// C = C.P^t
		OMD.mulin_left( C,TransposedBlasMatrix<BlasPermutation<size_t> >(Perm));
		// D = C.A (=B)
		OMD.mul(D, C, A);
		if (!MD.areEqual(D,B))
		  ret=false;
		...
		OMD.mul(D, C, A);
		// C = C.P^t
		OMD.mulin_left( D, TransposedBlasMatrix<BlasPermutation<size_t> >(Perm));
...
int main(int argc, char **argv)
{
	...
	commentator.start("OpenCLMatrixDomain test suite", "BlasMatrixDomain");
