...
// The mul & sqr routines use KarMul from ZZX, 
...
void ToModularRep(vec_long& x, const ZZ_p& a)
{
   ...
   MultiRem(&x[0], ZZ_pInfo->NumPrimes, rep(a), FFTPrime, ZZ_pInfo->tbl);
#elif (defined(NTL_TBL_REM))
   MultiRem(&x[0], ZZ_pInfo->NumPrimes, rep(a), FFTPrime, ZZ_pInfo->tbl1);
#else
   MultiRem(&x[0], ZZ_pInfo->NumPrimes, rep(a), FFTPrime);
#endif
}
...
void FromModularRep(ZZ_p& x, const vec_long& a)
{
   ...
   MultiMul(t, n, ZZ_pInfo->CoeffModP.elts(), 
            &a[0], ZZ_pInfo->mmsize);

   mul(s, q, ZZ_pInfo->MinusMModP);
   add(t, t, s);

   QuickRem(t, ZZ_pInfo->p);
   x.LoopHole() = t;
}
