...
GivensCache_QP::GivensCache_QP(long m, long n)
{
   ...
   long i;
   buf = new quad_floatptr[sz]; 
   if (!buf) Error("out of memory");
   for (i = 0; i < sz; i++)
      if (!(buf[i] = new quad_float[n+1])) Error("out of memory");
