...
static bool checkResult(const Field& F, Matrix& A, Vector& B, Vector& X, typename Field::Element& d)
{
...
void genData(const Field& F, Matrix& Mat, size_t bits, int seed = 0)
{
    ...
    // @fixme Why do we need a PAR_BLOCK here?
    PAR_BLOCK {
...
template <class Field, class Matrix, class Vector>
bool test_set(const Field& F, Vector& X2, Matrix& A, Vector& B)
{
    bool tag = false;
