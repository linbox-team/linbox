...
///////////////////////////////////////////////////////TEMPLATE/////////////////////////////////////////////////////////////
		...
		BlasVector<Givaro::ZRing<Integer> > & operator() ( BlasVector<Givaro::ZRing<Integer> > & num, Integer& den, Function& Iteration, PrimeIterator& primeg)
		{
            ...
            BlasVector<Domain> r(D);
//            Timer chrono;
bool isFirstTime[procs - 1];
float timeExec[procs - 1];
...
isFirstTime[i - 1]=true;
				...
				int poison_pills_left = procs - 1;
                int pp;
std::unordered_set<int> already_notified;
				while(poison_pills_left > 0 ){
std::cerr<<" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " << std::endl;
					int idle_process = 0;
                    r.resize (r.size()+2);

std::cerr<<" poison_pills_left:= "<< poison_pills_left << std::endl;
					//  receive the beginnin and end of a vector in heapspace
					_commPtr->recv(r.begin(), r.end(), MPI_ANY_SOURCE, 0); 
 std::cerr<<" poison_pills_left:= "<< poison_pills_left << std::endl;
std::cerr<<" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< " << std::endl;                    
...
if(count[idle_process - 1]>=0){
					//  send the tag
    			_commPtr->send(primes[idle_process - 1], idle_process);  
if(Builder_.terminated()){

count[idle_process - 1]--;

}else{
count[idle_process - 1]+=primes[idle_process - 1];
}
}
...
if(!Builder_.terminated()){
    if(isFirstTime[idle_process - 1]){
    timeExec[idle_process - 1] = r[r.size()-1];
    isFirstTime[idle_process - 1] = false;
    } else{
    if(timeExec[idle_process - 1] > r[r.size()-1] ){
    primes[idle_process - 1]++;poison_pills_left++;count[idle_process - 1]++;
    timeExec[idle_process - 1] = r[r.size()-1];
    }
    }
...
std::cerr<<"Proc("<<process<<") =============================END============================= " << std::endl;                
				return Builder_.result(num,den);
                
			}
			//  child process
			else{
            Timer chrono;float timeExec;
				int pp;
                LinBox::MaskedPrimeIterator<LinBox::IteratorCategories::HeuristicTag>   gen(process,procs);  
                ++gen; 
				//  get a prime, compute, send back start and end
				...
				while(true){
//std::cerr<<"Proc("<<process<<") >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>1>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " << std::endl;
					_commPtr->recv(pp, 0);
//std::cerr<<"Proc("<<process<<") <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<1<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< " << std::endl;
					if(pp == 0)
						break;
...
timeExec += chrono.usertime();
                    //Add corresponding prime number as the last element in the result vector
                    r.push_back(*gen);
r.push_back(timeExec);
//std::cerr<<"Proc("<<process<<") >>>>>>>>>>>>>>>>>>>>>>>>>>>>>2>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " << std::endl;
					_commPtr->send(r.begin(), r.end(), 0, 0);
//std::cerr<<"Proc("<<process<<") <<<<<<<<<<<<<<<<<<<<<<<<<<<<<2<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< " << std::endl;
}
...
#endif
//////////////////////////////////////Manually Fixed Task number distribution////////////////////////////////////////////
#if 0
		template<class Function, class PrimeIterator>
		BlasVector<Givaro::ZRing<Integer> > & operator() ( BlasVector<Givaro::ZRing<Integer> > & num, Integer& den, Function& Iteration, PrimeIterator& primeg)
		{
...
stop[i - 1]=false;
				...
				Builder_.initialize( D, Iteration(r, D) );
				int poison_pills_left = 2*(procs - 1);
                int pp;
std::unordered_set<int> already_notified;
				while(poison_pills_left > 0 ){
//std::cerr<<" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " << std::endl;
...
if(Builder_.terminated()){
    poison_pills_left--;
  primes[idle_process - 1]=0;//-primes[idle_process - 1];
//std::cerr<<" ================TIME2STOP================== " << std::endl;
}


					//  send the tag
if(!stop[idle_process - 1])    			_commPtr->send(primes[idle_process - 1], idle_process);  
if(!Builder_.terminated()) poison_pills_left++;

if(primes[idle_process - 1]==0 && !stop[idle_process - 1]){
 std::cerr<<"STOP signal sent 2 Proc("<<idle_process<<") ============= " << std::endl;
 stop[idle_process - 1]=true;
}
...
//std::cerr<<"Proc("<<process<<") <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<1<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< " << std::endl;
					if(pp == 0){
std::cerr<<"Proc("<<process<<") received STOP signal " << std::endl;
						break;
                    }
...
//std::cerr<<" ================TIME2STOP================== " << std::endl;
					...
					//  send the tag
if(!stop[idle_process - 1])    			_commPtr->send(primes[idle_process - 1], idle_process);  
if(!Builder_.terminated()) poison_pills_left+=primes[idle_process - 1]-1;
...
stop[i - 1]=false;
				...
				Builder_.initialize( D, Iteration(r, D) );
				int poison_pills_left = 1*(procs - 1);
                int pp;
std::unordered_set<int> already_notified;
				while(poison_pills_left > 0 ){
//std::cerr<<" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " << std::endl;
...
/*
    ...
    if(timeExec[idle_process - 1] > r[r.size()-1] ){
    primes[idle_process - 1]+=1;//poison_pills_left++;
    timeExec[idle_process - 1] = r[r.size()-1];
    }
    }
*/
