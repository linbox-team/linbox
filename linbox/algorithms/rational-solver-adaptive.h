...
namespace LinBox
{
	...
	struct RationalSolverAdaptiveClass {
		static SolverReturnStatus solveNonsingular(OutVector& num, typename IRing::Element& den, const BlasMatrix<IRing>& M, const InVector& b)
		{
			...
			RationalSolver<IRing, Field, PrimeIterator<RandomCategories::HeuristicTag>, NumSymNormTraits> numerical_solver;
			//RationalSolver<IRing, Field, PrimeIterator<RandomCategories::HeuristicTag>, NumSymOverlapTraits> numerical_solver;
			SolverReturnStatus ret;
			ret = numerical_solver. solve(num, den, M, b);

			if (ret != SS_OK) {
				RationalSolver<IRing, Field, PrimeIterator<RandomCategories::HeuristicTag>> solver;
