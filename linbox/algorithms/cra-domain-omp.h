...
namespace LinBox {
    ...
    struct ChineseRemainderOMP : public ChineseRemainderSeq<CRABase> {
        ...
        void solve_with_prime(PrimeIterator& m_primeiters, Function& Iteration, Domain& ROUNDdomains,
        ...
        */
        ...
        void compute_task(pFunc& pF, std::vector<PrimeIterator>& m_primeiters, Function& Iteration,
                          std::vector<Domain>& ROUNDdomains, std::vector<ElementContainer>& ROUNDresidues,
                          std::vector<CRABase>& vBuilders)
        {
...
#pragma omp single
            for (auto j = 1; j < Niter; j++) {
#pragma omp task
                solve_with_prime(m_primeiters[omp_get_thread_num()], Iteration, ROUNDdomains[j], ROUNDresidues[j],
                                 vBuilders[omp_get_thread_num()]);
            }
