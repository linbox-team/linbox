		...
		case Specifier::SINGULARITY_UNKNOWN:
			...
			case SS_SINGULAR:
				...
				case DixonTraits::DIOPHANTINE:
					{
						DiophantineSolver<RationalSolver<Ring,Field,PrimeIterator<RandomCategories::HeuristicTag>, DixonTraits> > dsolve(rsolve);
...
{ /*  Integer */
			 ...
			 const MethodTraits& m)
	{
		...
		PrimeIterator<RandomCategories::HeuristicTag> genprime( (unsigned) bits);
		RationalRemainder< EarlyMultipRatCRA< Givaro::Modular<double> > > rra(3UL);
		IntegerModularSolve<BB,Vector,MethodTraits > iteration(A, b, m);
		Integer den;
...
namespace LinBox {
		 ...
		 const Method::NumSymNorm & m)
	{
		...
		RationalSolver<Givaro::ZRing<Integer>, ZField, PrimeIterator<RandomCategories::HeuristicTag>, NumSymNormTraits> rsolver(ZZ);
