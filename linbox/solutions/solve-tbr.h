...
namespace LinBox
{
                  ...
                  const MethodTraits& m)
    {
        ...
        RationalRemainder2< VarprecCraBuilderEarlyMultip<Field> > rra(3UL);//using default RR method
                ...
                const Method::SparseElimination& m)
    {
        ...
        /** \brief solver specialization with the 2nd API and Method::Dixon over integer (no copying)
         */
    template <class Vector, class Ring>
    Vector& solve(Vector& x, typename Ring::Element &d,
                  ...
                  const RingCategories::IntegerTag tag, Method::Dixon& m)
    {
        ...
        typedef Givaro::Modular<double> Field;
        PrimeIterator<IteratorCategories::HeuristicTag> genprime(FieldTraits<Field>::bestBitSize(A.coldim()));
        RationalSolver<Ring, Field, PrimeIterator<IteratorCategories::HeuristicTag>, Method::Dixon> rsolve(A.field(), genprime);
        ...
        switch ( m.singular() ) {
            case Specifier::SINGULARITY_UNKNOWN:
                switch (A.rowdim() == A.coldim() ?
                    ...
                    case SS_SINGULAR:
                        switch (m.solution()){
                            case Method::Dixon::DETERMINIST:
                                status= rsolve.monolithicSolve(x, d, A, b, false, false, (int)m.maxTries(),
                                                               (m.certifyInconsistency? SL_LASVEGAS: SL_MONTECARLO));
                                break;
                            case Method::Dixon::RANDOM:
                                status= rsolve.monolithicSolve(x, d, A, b, false, true, (int)m.maxTries(),
                                                               (m.certifyInconsistency? SL_LASVEGAS: SL_MONTECARLO));
                                break;
                            case Method::Dixon::DIOPHANTINE:
                            {
                                DiophantineSolver<RationalSolver<Ring,Field,PrimeIterator<IteratorCategories::HeuristicTag>, Method::Dixon> > dsolve(rsolve);
                ...
                Method::Dixon& m)
    {
        ...
        typedef Givaro::Modular<double> Field;
        PrimeIterator<IteratorCategories::HeuristicTag> genprime(FieldTraits<Field>::bestBitSize(A.coldim()));
        RationalSolver<Ring, Field, PrimeIterator<IteratorCategories::HeuristicTag>, Method::SparseElimination> rsolve(A.field(), genprime);
        SolverReturnStatus status = SS_OK;
...
{ /*  Integer */
                     ...
                     )
    {
        ...
        RationalCraDistributed< RationalCraBuilderEarlyMultip< Givaro::Modular<double> > > mpicra(3UL, C);
        ...
        // 0.7213475205 is an upper approximation of 1/(2log(2))
        PrimeIterator<IteratorCategories::HeuristicTag> genprime((unsigned int)( 26 -(int)ceil(log((double)A.rowdim())*0.7213475205)));
            //         RationalCra< Givaro::Modular<double> > rra((double)
        ...
        IntegerModularSolve<BB,Vector,MyMethod> iteration(A, b, M);
        RationalCra< RationalCraBuilderEarlyMultip< Givaro::Modular<double> > > rra(3UL);
             ...
             const Method::IML& m)
    {
        ...
        switch (m.routine()) {
            ...
            case (2) : { /*  certified */
                ...
                if ( !m.reduced() ) {
                    IML::certSolveMP(m.certifyInconsistency,
                                     (long) B.rowdim(),(long)B.coldim(),
                                     mp_A, mp_b, mp_N, mp_D,
                                     mp_NZ, mp_DZ);
                }
                else {
                    IML::certSolveRedMP(m.certifyInconsistency,
...
namespace LinBox {
    ...
    solveNum(BlasVector<Givaro::ZRing<Integer>>& x, Givaro::ZRing<Integer>::Element & d,
             const BlasMatrix<Givaro::ZRing<Integer>>& B, const BlasVector<Givaro::ZRing<Integer>>& b,
             const Method::NumericSymbolicOverlap & m)
    {
        THIS_CODE_COMPILES_BUT_IS_NOT_TESTED; // NOT MUCH
...
namespace LinBox {
    ...
    solveNum(BlasVector<Givaro::ZRing<Integer>>& x, Givaro::ZRing<Integer>::Element & d,
             const BlasMatrix<Givaro::ZRing<Integer>>& B, const BlasVector<Givaro::ZRing<Integer>>& b,
             const Method::NumericSymbolicNorm & m)
    {
            //THIS_CODE_COMPILES_BUT_IS_NOT_TESTED; // NOT MUCH
        ...
        RationalSolver<Givaro::ZRing<Integer>, ZField, PrimeIterator<IteratorCategories::HeuristicTag>, Method::NumericSymbolicNorm> rsolver(ZZ);
