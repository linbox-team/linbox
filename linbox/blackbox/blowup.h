...
public:
	typedef Blowup<_Blackbox> Self_t;

   using Father_t =  _Blackbox;
   using Field = typename Father_t::Field;
   using Element = typename Father_t::Element;
   using Matrix = typename DenseMatrix<Field>;
   using Vector_t = BlasVector<Field>;

	/// \brief cstor setting this to the blowup of xI-A modulo f
	Blowup(_Blackbox & baseBB, Vector_t & modulusPoly) 
   : _Blackbox(&baseBB), _f(modulusPoly) {}
         
	~Blowup(){}

   BBType bbTag() const { return other; } // for now

   Father_t & baseBB() const { return *this; }
