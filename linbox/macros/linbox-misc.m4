...
AC_ARG_WITH(all,
	    [ --with-all= <path>|yes|no
					Use all external packages.
					If the argument is no, you not sure that all libraries are 
					reachable with the default path.
					If the argument is yes or <empty>, that means that all libraries 
					are reachable with the default path.
					Otherwise add <path> to default path and enable all external packages.
	    ],
	    [if test "$withval" = yes ; then
			check_all="yes"
