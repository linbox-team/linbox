...
namespace LinBox
{
	...
	PackedPermutation<_Uint>::PackedPermutation() :
	       	r_(0),n_(-1),P_(0),Q_(0),cleaned_(true)
	{};
	       	...
	       	r_(r), n_(-1),P_(0),Q_(0),cleaned_(clean)
	{
		...
		if (clean)
		{
			...
			if ((rr == 0) && (P[0] == 0)) { r_ = 0 ; n_ = 0  ; return ; }
			r_ = rr+1 ;
	...
	PackedPermutation<_Uint>::PackedPermutation(const MatrixPermutation<_Uint> & P, bool clean) :
	       	r_(P.getSize()), n_(P.getSize()),P_(P.getSize()),Q_(P),cleaned_(clean)
	{
	...
	PackedPermutation<_Uint>::Convert (const MatrixPermutation<_Uint> & P)
	{
		// already computed
		if (n_ != 0 && Q_.size()>0) return ;
	...
	/* ****** */
	template<class _Uint>
	std::ostream & PackedPermutation<_Uint>::write (std::ostream & o, bool Lapack)
	{
		if (Lapack) {
			o << '['  ;
			for (_Uint i = 0 ; i < r_ ; ++i)
...
namespace LinBox
{
	...
	/* ****** */
	template<class _Uint>
	std::ostream & MatrixPermutation<_Uint>::write (std::ostream & o)
	{
		o << '['  ;
		for (_Uint i = 0 ; i < n_ ; ++i)
		{ o << P_[i]  ; if (i< n_-1) o << ','; }
