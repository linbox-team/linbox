...
/* linbox/src/blackbox/butterfly.h
...
namespace LinBox
{
					...
					Commentator &Comm = Commentator ())
	{
		...
		Comm.start ("Butterfly::setButterfly", LVL_NORMAL, INTERNAL_DESCRIPTION);
		...
		if (x.size () <= 1) {
			Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
				<< "No switches needed. Returning with empty vector.";

			return std::vector<bool> ();
		}

		Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
			<< "Counting the number of switches that exist.";
		...
		Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
			<< "There are a total of " << s << " switches";
 
		// Set largest power of 2 in decomposition of n = x.size ()
		size_t n_p (*n_vec.rbegin ());
 
		Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
			<< "Found largest power of 2 in decomposition of " << n
			<< " as n_p = " << n_p;
		...
		if (n == n_p) {
			n_p /= 2;	  // >> is not portable!
				...
				Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
					<< "n = " << n << " is a power of two.  "
					<< "Resetting n_p to be half of n: n_p = " << n_p;
		}
		...
		if ( (s_1 + s_3) == r_1 ) {
			...
			if (j < (n - n_p)) {
				Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
					<< "  A: j < (n - n_p).  j_1 = j = " << j << ", j_2 = 0";

				y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), j);
				y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), 0);
 
			} else {
				Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
					<< "  A: j >= (n - n_p).  j_1 = 0, j_2 = j - (n - n_p) = "
					<< j - (n - n_p);

				// This case cannot occur for n != 2*n_p because j != 0

				y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), 0);
				y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), j - (n - n_p));
			}
				...
				if (j < (n - n_p)) {
					Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
						<< "  A: j < (n - n_p).  j_1 = j, j_2 = 2*n_p + j + r_1 - n = "
						<< 2*n_p + j + r_1 - n;
					// In this case, s_1 > 0, so s_3 = 0, and wrap-around cannot occur.

					y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), j);
					y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), 2*n_p + j + r_1 - n);
					...
					y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), j + r - n - r_1);
					y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), j - (n - n_p));
				...
				if (j < (n - n_p)) {
					Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
						<< "  A: j < (n - n_p).  j_1 = j = " << j
						<< ", j_2 = j + r_1 - n + n_p = " << j + r_1 - n + n_p;
					// In this case, s_1 > 0, so s_3 = 0, and wrap-around cannot occur.

					y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), j);
					y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), j + r_1 - n + n_p);
					...
					y_1 = set_Butterfly (std::vector<bool>(x.begin (), x.begin () + (n - n_p)), j + r - n_p - r_1);
					y_2 = set_Butterfly (std::vector<bool>(x.begin () + (n - n_p), x.end ()), j - (n - n_p));
			...
			Comm.report (INTERNAL_DESCRIPTION, LVL_NORMAL)
				<< "The output vector for n = " << n << " has " << y.size ()
				<< " entries."
			...
			Comm.stop (LVL_NORMAL, INTERNAL_DESCRIPTION, LONG_TIMING);
