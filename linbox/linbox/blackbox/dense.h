...
namespace LinBox
{
	...
	struct MatrixTraits< Protected::DenseMatrix<Field> >
	{
		typedef Protected::DenseMatrix<Field> MatrixType;
		typedef typename MatrixCategories::RowMatrixTag MatrixCategory;
	};

	template <class Field>
	struct MatrixTraits< const Protected::DenseMatrix<Field> > {
		typedef const Protected::DenseMatrix<Field> MatrixType;
		typedef typename MatrixCategories::RowMatrixTag MatrixCategory;
	};
	...
	class DenseMatrixFactory : public BlackboxFactory<Field,Protected::DenseMatrix<Field> > {
	private:
		const Protected::DenseMatrixBase<BElement>& _A;

	public:
		DenseMatrixFactory( const Protected::DenseMatrixBase<BElement> &A ) :
			_A(A)
		{}

		Protected::DenseMatrix<Field>* makeBlackbox( const Field& F );
