...
#include "linbox/gen_superlu/sp_coletree.h" // A. Duran 11/8/02

int check_perm(char *what, int n, int *perm);
template <class Field>
void
sp_preorder(char *refact,  SuperMatrix<Field> *A, int *perm_c, 
	    int *etree, SuperMatrix<Field> *AC, Field& F)
{
/*
 * Purpose
 * =======
 ...
 */
    ...
    AC->Store = (typename Field::Element *) SUPERLU_MALLOC( sizeof(NCPformat<Field>) ); // A.Duran
    ACstore = (NCPformat<Field> *)AC->Store;
...
int check_perm(char *what, int n, int *perm)
{
	...
	if ( marker[perm[i]] == 1 || perm[i] >= n ) {
	    cout<<what<<": Not a valid PERM["<<i<<"] = "<<perm[i]<<"\n";
	    cout<<"check_perm";
	} else {
	    marker[perm[i]] = 1;
	}
