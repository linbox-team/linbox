       ...
       )
{
  ...
  for (i = 0; i < nseg; i++) {
	...
	while ( kmin <= kmax ) {
	  ...
	  if ( perm_r[lsub[kmax]] == EMPTY ) 
	    ...
	    if ( movnum ) {
	      ...
	      F.assign(utemp, lusup[minloc]);
	      F.assign(lusup[minloc], lusup[maxloc]);
	      F.assign(lusup[maxloc], utemp);

#ifdef CHK_PRUNE
      	cout << "At pruneL lusup[maxloc]"<< lsub[kmin] << "\n";
#endif
	    }
	...
	printf("    After pruneL(),using col %d:  xprune[%d] = %d\n", 
	       jcol, irep, kmin);
#endif
      } /* if do_prune */
