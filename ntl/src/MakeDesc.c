...
void print_BB_sqr_code(FILE *f, long n)
{
   ...
   for (i = 8; i < n; i += 8) {
      if (2*(i+8) <= n) {
         ...
         fprintf(f, "u_long t=sqrtab[(a>>%ld)&255];\\\n", i);
...
main()
{
...
#ifdef NTL_LONG_LONG
   fprintf(stderr, "NTL_LONG_LONG ");
#endif

#ifdef NTL_LONG_LONG_TYPE
   fprintf(stderr, "NTL_LONG_LONG_TYPE ");
#endif

#ifdef NTL_CPLUSPLUS_ONLY
   fprintf(stderr, "NTL_CPLUSPLUS_ONLY ");
#endif


#ifdef NTL_X86_FIX
   fprintf(stderr, "NTL_X86_FIX ");
#endif

#ifdef NTL_NO_X86_FIX
   fprintf(stderr, "NTL_NO_X86_FIX ");
#endif

#ifdef NTL_AVOID_FLOAT
   fprintf(stderr, "NTL_AVOID_FLOAT ");
#endif

#ifdef NTL_AVOID_BRANCHING
   fprintf(stderr, "NTL_AVOID_BRANCHING ");
#endif

#ifdef NTL_FFT_PIPELINE
   fprintf(stderr, "NTL_FFT_PIPELINE ");
#endif

#ifdef NTL_SINGLE_MUL
   fprintf(stderr, "NTL_SINGLE_MUL ");
#endif

#ifdef NTL_FAST_INT_MUL
   fprintf(stderr, "NTL_FAST_INT_MUL ");
#endif

#ifdef NTL_TBL_REM
   fprintf(stderr, "NTL_TBL_REM ");
#endif

#ifdef NTL_NO_INIT_TRANS
   fprintf(stderr, "NTL_NO_INIT_TRANS ");
#endif

#ifdef NTL_RANGE_CHECK
   fprintf(stderr, "NTL_RANGE_CHECK ");
#endif
   ...
   if (dp1 > dp && !AutoFix) {
      ...
      fprintf(stderr, "Set this flag by editing the file ntl_config.h.\n");
   ...
   if (warnings) {
      ...
      c = getchar();
      if (c == 'n' || c == 'N') {
         fprintf(stderr, "Make the necessary changes to the makefile and/or ntl_config.h,\n");
         fprintf(stderr, "then type 'make clobber' and then 'make' to install.\n\n\n");
         return 1;
      }
   }
