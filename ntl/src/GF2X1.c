...
void PlainDivRem(GF2X& q, GF2X& r, const GF2X& a, const GF2X& b)
{
   ...
   for (i = 1; i <= min(dq, NTL_BITS_PER_LONG-1); i++) 
      MulByX(stab[((u_long)(posb+i))%NTL_BITS_PER_LONG], 
             stab[((u_long)(posb+i-1))%NTL_BITS_PER_LONG]);

   u_long *stab_ptr[NTL_BITS_PER_LONG];
   long stab_cnt[NTL_BITS_PER_LONG];

   for (i = 0; i <= min(dq, NTL_BITS_PER_LONG-1); i++) {
      WordVector& st = stab[((u_long)(posb+i))%NTL_BITS_PER_LONG].xrep;
      long k = st.length();
      stab_ptr[((u_long)(posb+i))%NTL_BITS_PER_LONG] = &st[k-1];
      stab_cnt[((u_long)(posb+i))%NTL_BITS_PER_LONG] = -k+1;
   }

   q.xrep.SetLength(sq);
   u_long *qp = q.xrep.elts();
   for (i = 0; i < sq; i++)
      qp[i] = 0;
...
void build(GF2XModulus& F, const GF2X& f)
{
   ...
   if (F.method == GF2X_MOD_SPECIAL) {
      if (!F.stab_cnt) F.stab_cnt = new long[NTL_BITS_PER_LONG];
      long *stab_cnt = F.stab_cnt;
      if (!stab_cnt) Error("out of memory");

      if (!F.stab1) F.stab1 = new u_long[2*NTL_BITS_PER_LONG];
      u_long *stab1 = F.stab1;
      ...
      for (i = 1; i < NTL_BITS_PER_LONG; i++) {
         long kk0 = ((u_long)(posb+i-1))%NTL_BITS_PER_LONG;
         long kk1 = ((u_long)(posb+i))%NTL_BITS_PER_LONG;
      ...
      if (!F.stab_ptr) F.stab_ptr = new u_long_ptr[NTL_BITS_PER_LONG];
      u_long **stab_ptr = F.stab_ptr;
   ...
   stab_cnt(0), stab_ptr(0), stab1(0), tracevec(F.tracevec)
{
   if (method == GF2X_MOD_SPECIAL) {
      long i;
      stab1 = new u_long[2*NTL_BITS_PER_LONG];
      if (!stab1) Error("GF2XModulus: out of memory");
      for (i = 0; i < 2*NTL_BITS_PER_LONG; i++)
         stab1[i] = F.stab1[i];
      stab_cnt = new long[NTL_BITS_PER_LONG];
      ...
      if (F.stab_ptr) {
         stab_ptr = new u_long_ptr[NTL_BITS_PER_LONG];
         if (!stab_ptr) Error("GF2XModulus: out of memory");
      
         for (i = 0; i < NTL_BITS_PER_LONG; i++) {
            WordVector& st = stab[((u_long)(posn+i))%NTL_BITS_PER_LONG].xrep;
            long k = st.length();
            stab_ptr[((u_long)(posn+i))%NTL_BITS_PER_LONG] = &st[k-1];
            stab_cnt[((u_long)(posn+i))%NTL_BITS_PER_LONG] = -k+1;
         }
      }
   }
}
...
GF2XModulus& GF2XModulus::operator=(const GF2XModulus& F)
{
   ...
   if (method == GF2X_MOD_SPECIAL) {
      long i;
      if (!stab1) stab1 = new u_long[2*NTL_BITS_PER_LONG];
      if (!stab1) Error("GF2XModulus: out of memory");
      for (i = 0; i < 2*NTL_BITS_PER_LONG; i++)
         stab1[i] = F.stab1[i];
      if (!stab_cnt) stab_cnt = new long[NTL_BITS_PER_LONG];
      if (!stab_cnt) Error("GF2XModulus: out of memory");
      for (i = 0; i < NTL_BITS_PER_LONG; i++)
         stab_cnt[i] = F.stab_cnt[i];
   }
      ...
      if (F.stab_ptr) {
         if (!stab_ptr) stab_ptr = new u_long_ptr[NTL_BITS_PER_LONG];
         if (!stab_ptr) Error("GF2XModulus: out of memory");
...
void RightShiftAdd(GF2X& c, const GF2X& a, long n)
{
   ...
   u_long *cp = c.xrep.elts();
   const u_long *ap = a.xrep.elts();
...
inline void swap(u_long_ptr& a, u_long_ptr& b)  
{  u_long_ptr t;  t = a; a = b; b = t; }
...
void GCD(GF2X& d, const GF2X& a_in, const GF2X& b_in)
{
   ...
   u_long *ap = a.xrep.elts();
   u_long *bp = b.xrep.elts();
                   ...
                   const vec_GF2X& H, long n, WordVector& t)
{
   ...
   const u_long *vp = &v.xrep[w_low];
   u_long msk = 1UL << b_low;
   u_long vv = *vp;
