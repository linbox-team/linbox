...
START_FIX
   ...
   // arithmetic, the following prevents int(xhi) from overflowing.

   if (n > 0)
      xlo = double(n+int(-xhi));
   else
      xlo = double(n-int(xhi));
...
START_FIX
   ...
   // Because we are assuming 2's compliment longeger
   // arithmetic, the folxlowing prevents long(xhi) from overfxlowing.
...
{
   if (p < 1) Error("quad_float: output precision too small");
