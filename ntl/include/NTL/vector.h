...
#define NTL_vector_ff_decl(T,vec_T)  \
class vec_T {  \
private:  \
   const T##InfoT *field_def;  \
   T *l__rep;  \
  \
   void RangeError(long l__i) const;  \
  \
public:  \
  ...
  \
   ...
   long fixed() const { return l__rep && NTL_VEC_HEAD(l__rep)->fixed; } \
  \
   const T##InfoT *field () const { return field_def; } \
  \
   T& operator[](long l__i)   \
   {  \
      NTL_RANGE_CHECK_CODE  \
...
#define NTL_vector_ff_impl(T,vec_T) NTL_vector_default(T) NTL_vector_ff_impl_plain(T,vec_T)  
...
#define NTL_vector_ff_impl_plain(T,vec_T)  \
 \
void vec_T::SetLength(long l__n)   \
{   \
   long l__m;  \
...
#define NTL_eq_vector_ff_impl(T,vec_T) \
long operator==(const vec_T& l__a, const vec_T& l__b) \
{  \
   long l__n = l__a.length();  \
   if (l__b.length() != l__n) return 0;  \
   ...
   const T* l__bp = l__b.elts(); \
   long l__i;  \
   if (l__a.field () != l__b.field ()) return 0;  \
   for (l__i = 0; l__i < l__n; l__i++) if (l__ap[l__i] != l__bp[l__i]) return 0;  \
   return 1;  \
} \
long operator!=(const vec_T& l__a, const vec_T& l__b) \
