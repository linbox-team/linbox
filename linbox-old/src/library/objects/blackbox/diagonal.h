...
namespace LinBox
{
	...
	class Diagonal : public Blackbox_archetype<Vector>
	{
		 ...
		 * @param F	LinBox field in which ot do arithmetic
		...
		/** Constructor from field and an random number generator for
		 * that field. Generates a random diagonal matrix.
		 * @param F	LinBox field in which ot do arithmetic
		 * @param iter  Linbox random number iterator for the given field
		 */
		Diagonal (const Field F, const Field::randIter &iter);
	...
	template <class Field, class Vector>
	inline Diagonal<Field, Vector, vector_categories::dense_vector_tag>
		::Diagonal (const Field F, const int n, const Field::randIter &iter)
		: _F (F), _n (n)
	{
		int i;
		...
		::apply (Vector &y, const Vector& x) const
	{
		...
		return y;
	} // Vector& Diagonal<dense_vector_tag>::apply (Vector &, const Vector&) const
		...
		::apply (Vector &y, const Vector& x) const
	{
		...
		size_t i;
		element zero = _F.zero (), entry = _F.zero ();
	...
	} // Vector& Diagonal<sparse_sequence_vector_tag>::apply (Vector &, const Vector&) const
