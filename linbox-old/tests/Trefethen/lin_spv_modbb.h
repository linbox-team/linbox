...
// Time-stamp: <28 Jan 02 18:06:13 Jean-Guillaume.Dumas@imag.fr> 
// =========================================================
...
class SparseBlackBoxModularDom {
public:
    ...
    typedef          SparseBlackBoxModularDom< Domain, I >             Self_t;
...
public:
        ///-- Default cstors:
    SparseBlackBoxModularDom() : _domain(),_nz_elem(0) {};
    SparseBlackBoxModularDom(const Domain& D) : _domain(D),_nz_elem(0) { }
    SparseBlackBoxModularDom(const Domain& D, char * mat_file) : _domain(D),_nz_elem(0) { read(mat_file) ; }
    
        ///-- Cstor of recopy: compiler's generated
    SparseBlackBoxModularDom(const Self_t& M) : _domain(M._domain),_row_dim(M.n_row()),_col_dim(M.n_col()),_nz_elem(0),_container(M._container) {}
    ...
    OutMatrix& Apply(OutMatrix& res, const InMatrix& vect, const Rep& ca ) const {
        ...
        for(k=n_row(); k-- ; )
            for( res[k]=_domain.zero, i=ca[k].size(); i-- ; ) {
                toto = ca[k][i];
                if (_domain.isOne( toto.getvalue() ) )
                    _domain.addin(res[k],vect[toto.getindex()]);
                else
    ...
    OutMatrix& ApplyTrans(OutMatrix& res, const InMatrix& vect, const Rep& ca) const {
        ...
        for(k=ca.size(); k-- ; )
            for(i=ca[k].size(); i-- ; ) {
                toto = ca[k][i];
                if ( _domain.isOne( toto.getvalue() ) )
                    _domain.addin( res[ toto.getindex() ], vect[k]);
                else
