...
% we haven't committed to this as a MM variant, should we?
...
% \end{description}
	 ...
	 \par
%
This is a common minimal interface to all our matrices that all
of our algorithms can use. Additional elements to a container can be added and flagged
%bb removed ,
with a trait. This model also ensures the minimal ``linear application'' use of a
matrix.  This \applin interface is shared by \eg: \emph{dense}
containers ({\scsf{Blas}-like,\ldots); \emph{permutations} containers (compressed
\scsf{Lapack}, cycles); \emph{sparse} containers (based on
...
a basic operation in linear algebra that needs to be extremely efficient (\Cref{sec:matmul}).
%
%
% \par
%
