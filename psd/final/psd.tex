...
%convey the same information, where $\rank{A}$ means the rank of $A$, $\dim(A)$ means 
%the order of the matrix.
%This is true since all eigenvalues of a real symmetric matrix are real.
...
i%We call such vectors $\signature$-revealing vectors for the matrix.
...
\item
If the nonzero eigenvalues of $A$ are distinct,
$\signature(A) = \signature(shifted minpoly{A})$.\\
...
%$x^k \cdot$ minmal polynomial of $A$ for $k$ such
...
of leading principal minors up to the first which is zero mod $p$.  
%up to the 
...
%When $p$ is good and $X$ is nonzero (so $k < \rank{A}$),
...
%
%Fixme: There should be something in here about the bad imaging, mapping from $\Z$ to $\Z_p$.
%
...
The blackbox step cost is sensitive to $e$, the cost of the single matrix-by-vector
...
For $\M{\nu}{7168}$, DSLU requires explicit use of file storage of the expanded matrix
