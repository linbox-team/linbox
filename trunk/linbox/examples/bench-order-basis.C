...
void bench_sigma(const Field& F, const RandIter& Gen, size_t n, size_t d) { 
	...
	MatrixP A(F,n,n,d);
	MatrixP C(F,n,n,2*d-1);
	for (size_t k=0;k<d;++k){
		for (size_t i=0;i<n;++i)
			for (size_t j=0;j<n;++j){
				Gen.random(A.ref(i,j,k));
			}
	}
	PolynomialMatrixMulDomain<Field> PMD(F);
	chrono.start();	
	...
	BlockCoppersmithDomain<MatrixDomain<Field>,  MatrixP> BCD(BMD, &Serie, n*d/2);
	std::vector<BlasMatrix<Field> > gen;
	std::vector<size_t> deg;
	chrono.start();
	deg=BCD.right_minpoly(gen);		
	chrono.stop();
	std::cout << "Yuhasz code  : " <<chrono.usertime()<<endl;
#endif
...
void runTest(const Field& F, size_t n, long b, long d, long seed, string test){ 
	...
	if (test == "bench" || test == "all"){
		cout<<"## bench: order time(PM-Basis) time(oPM-Basis) time(matmul) time(MBasis)"<<endl;;
		bench_sigma(F,G,n,d);
	}
	if (test == "longbench"){
		cout<<"## matrix dimension is :"<<n<<endl;
		cout<<"## bench: order time(PM-Basis) time(oPM-Basis) time(matmul) time(MBasis)"<<endl;;
		for (long i=64;i<=d;i*=2){
			bench_sigma(F,G,n,i);
		}
	}
}
int main(int argc, char** argv){
	...
	static Argument args[] = {
		...
		{ 't', "-t t", "Choose the targeted test {all,check,bench,longbench}", TYPE_STR, &test},
		END_OF_ARGUMENTS
	};	
