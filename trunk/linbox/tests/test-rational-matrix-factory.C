...
static bool testDiagonalMatrix (size_t n)
{
	...
	GMPRationalField Q;
	Protected::DenseMatrix<GMPRationalField > A(Q,n,n);
	integer lcm_n=1;
	...
	RationalMatrixFactory<PID_integer, GMPRationalField, Protected::DenseMatrix<GMPRationalField > > FA(&A);
	...
	PID_integer Z;
	Protected::DenseMatrix<PID_integer> Aprim(Z,n,n);
	Protected::DenseMatrix<PID_integer> Atilde(Z,n,n);
