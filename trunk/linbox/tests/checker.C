...
void build_n_run(string s, counts& cnt, int flag = 2) ;
...
//BASIC_TESTS
	...
	build_n_run("test-smith-form", /*"make fails, bds responsible", */counter, flag);
	build_n_run("test-smith-form-local", counter, flag); //"intermittent failures");
...
//	if (flag > 0) cout << "	Givaro tests" << endl;
	...
	// needs output cleanup.  Resolve whether a benchmark or a test.
	no_build_n_run("test-solve-nonsingular", counter, flag);
#endif
	...
	no_build_n_run("test-toeplitz-det", counter, flag);
	...
	no_build_n_run("test-charpoly", counter, flag, "intermittent inf loop, bb or cp responsible?");
	no_build_n_run("test-rat-charpoly", counter, flag, "infinite loop, cp responsible?");
	...
	no_build_n_run("test-zo", counter, flag, "half baked, bds responsible");
