...
#pragma message "#warning  deprecated -- this is replaced by benchmark-fields.C"
/* tests/test-fields.C
...
//	std::cout << (iter/(int)vectorSize) << " dd " << timer.time() << std::endl;;
...
if (fulltest) {
	...
	for( i = 0; i < 9; i++ ) {
		...
		array[(size_t)i] = iter / (t > 0 ? (t * 1000000) : 0) ;
	}
