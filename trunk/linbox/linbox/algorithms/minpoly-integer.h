...
namespace LinBox
{
	...
	public:
		...
		static Poly& minPolyBlas (Poly& y, const Protected::DenseMatrix<Ring>& M);

		template <class Poly, class Ring>
		static Poly& minPolyBlas (Poly& y, const Protected::DenseMatrix<Ring>& M, int degree);

		template <class Ring>
		static int minPolyDegreeBlas (const Protected::DenseMatrix<Ring>& M, int n_try = 1);
	};
	...
	template <class _Integer, class _Field>
	template <class Poly, class Ring>
	Poly& MinPolyBlas<_Integer, _Field>::minPolyBlas (Poly& y, const Protected::DenseMatrix<Ring>& M)
	{
	...
	template <class _Integer, class _Field>
	template <class Poly, class Ring>
	Poly& MinPolyBlas<_Integer, _Field>::minPolyBlas (Poly& y, const Protected::DenseMatrix<Ring>& M, int degree)
	{
		...
		Element* p;
		typename Protected::DenseMatrix<Ring>::ConstRawIterator raw_p;
	...
	template <class _Integer, class _Field>
	template <class Ring>
	int MinPolyBlas<_Integer, _Field>::minPolyDegreeBlas (const Protected::DenseMatrix<Ring>& M, int n_try)
	{
		size_t n = M. rowdim();
		int degree = 0;
		Element* FA = new Element [n*n];
