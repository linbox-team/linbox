...
namespace LinBox
{
	...
	public:
			...
			,one((Element)1),zero((Element)0),mOne(Element(modulus-(Element)1))
		{
			...
			if(value <= 1) throw PreconditionFailed(LB_FILE_LOC,"modulus must be > 1");
			integer max;
			if(value > FieldTraits< Modular<Element> >::maxModulus(max)) throw PreconditionFailed(LB_FILE_LOC,"modulus is too big");
#endif
		}
	...
	private:
		...
		static void XGCD(Element& d, Element& s, Element& t, Element a, Element b)
		{
			...
			if (a < 0) {
#ifdef DEBUG
				if (a < -LINBOX_MAX_INT8) throw PreconditionFailed(LB_FILE_LOC,"XGCD: integer overflow");
#endif
				a = Element(-a);
				aneg = 1;
			}
			...
			if (b < 0) {
#ifdef DEBUG
				if (b < -LINBOX_MAX_INT8) throw PreconditionFailed(LB_FILE_LOC,"XGCD: integer overflow");
#endif
				b = (Element)-b;
				bneg = 1;
			}
	...
	template <>
	class DotProductDomain<Modular<int8_t> > : public virtual VectorDomainBase<Modular<int8_t> > {
	...
	public:
		...
		using VectorDomainBase<Modular<int8_t> >::field;
	protected:
