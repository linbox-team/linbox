...
#include "linbox/gen_superlu/gstrf.h"
#include "linbox/gen_superlu/gstrs.h"
#include "linbox/gen_superlu/sp_ienv.h"

template <class Field>
void
gssv(SuperMatrix<Field> *A, int *perm_c, int *perm_r, SuperMatrix<Field> *L,
      SuperMatrix<Field> *U, SuperMatrix<Field> *B, int *info, int *rank, Field& F)
{
/*
 * Purpose
 * =======
 *
 * GSSV solves the system of linear equations A*X=B, using the
 * LU factorization from GSTRF. It performs the following steps:
 *
 ...
 */
    ...
    SuperMatrix<Field> *AA; /* A in NC format used by the factorization routine.*/
    SuperMatrix<Field> AC; /* Matrix postmultiplied by Pc */
    int lwork = 0, *etree, i;
    
    /* Set default values for some parameters */
    typename Field::Element diag_pivot_thresh;
    typename Field::Element drop_tol;
    F.init(diag_pivot_thresh, 1);
    F.init(drop_tol, 0);
    ...
    *trans = 'N';
    ...
    if ( A->Stype == NR ) {
	NRformat<Field> *Astore = (NRformat<Field> *)A->Store;
	AA = (SuperMatrix<Field> *) SUPERLU_MALLOC( sizeof(SuperMatrix<Field>) );
	FCreate_CompCol_Matrix(AA, A->ncol, A->nrow, (int)Astore->nnz, 
			       (typename Field::Element *)Astore->nzval, (int *)Astore->colind, 
			       (int *)Astore->rowptr, NC, A->Dtype, A->Mtype, F);
	*trans = 'T';
    ...
    gstrf(refact, &AC, diag_pivot_thresh, drop_tol, relax, panel_size, etree, NULL, lwork, perm_r, perm_c, L, U, info, rank, F); // A.Duran 8/12/2002
    utime[FACT] = SuperLU_timer_() - t1;

    t1 = SuperLU_timer_();

    cout << "info "<< *info <<"\n";
    // if ( *info == 0 ) { // A. Duran excluded this condition
    /* Solve the system A*X=B, overwriting B with X. */
    gstrs(trans, L, U, perm_r, perm_c, B, info, F); // A.Duran 8/12/2002
    // }
