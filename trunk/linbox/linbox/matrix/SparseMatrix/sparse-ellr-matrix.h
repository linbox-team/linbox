...
#ifndef __LINBOX_sparse_matrix_sparse_ellr_matrix_H
#define __LINBOX_sparse_matrix_sparse_ellr_matrix_H
...
namespace LinBox
{
	...
	class SparseMatrix2<_Field, SparseMatrix2Format::ELL_R > {
	public :
		...
		typedef SparseMatrix2Format::ELL_R         Storage ; //!< Matrix Storage Format
		 ...
		 */
		//@{
		SparseMatrix2<_Field, SparseMatrix2Format::ELL_R> () :
		...
		SparseMatrix2<_Field, SparseMatrix2Format::ELL_R> (const _Field & F) :
			_rownb(0),_colnb(0)
			,_maxc(0)
			,_nbnz(0),
			_colid(0),_data(0)
		...
		SparseMatrix2<_Field, SparseMatrix2Format::ELL_R> (const _Field & F, size_t m, size_t n) :
			_rownb(m),_colnb(n)
			,_maxc(0)
			,_nbnz(0)
			,_colid(0),_data(0)
		...
		SparseMatrix2<_Field, SparseMatrix2Format::ELL_R> (const SparseMatrix2<_Field, SparseMatrix2Format::CSR> & S) :
		...
		// XXX only for CSR
		template<typename _Tp1, typename _Rw1 = SparseMatrix2Format::ELL_R>
		struct rebind ;

		template<typename _Tp1>
		struct rebind<_Tp1/*  ,SparseMatrix2Format::COO */ > {
			typedef SparseMatrix2<_Tp1, SparseMatrix2Format::ELL_R> other;
		...
		SparseMatrix2 (const SparseMatrix2<_Tp1, _Rw1> &S, const Field& F) :
			...
			,_rowid(S.rowdim(),0)
		{
			typename SparseMatrix2<_Tp1,_Rw1>::template rebind<Field,SparseMatrix2Format::ELL_R>()(*this, S);
		}
		...
		SparseMatrix2<_Field, SparseMatrix2Format::ELL_R> (const _Field & F, VectStream & stream) :
			...
			,_rowid(_rownb,0)
		{
			SparseMatrix2<_Field,SparseMatrix2Format::CSR> Tmp(F,stream);
			importe(Tmp);
		}
		...
		void importe(const SparseMatrix2<_Field,SparseMatrix2Format::ELL_R> &S)
		{
		...
		exporte(SparseMatrix2<_Field,SparseMatrix2Format::CSR> &S) const
		{
		...
		*/
		void transposeIn()
		{
			SparseMatrix2<_Field,SparseMatrix2Format::ELL_R> Temp(*this);
			Temp.transposeIn();
			importe(Temp);
		}
		 ...
		 */
		SparseMatrix2<_Field,SparseMatrix2Format::ELL_R> &
		transpose(SparseMatrix2<_Field,SparseMatrix2Format::ELL_R> &S)
		{
			S.importe(*this);
			S.transposeIn();
			return S;
...
#endif // __LINBOX_sparse_matrix_sparse_ellr_matrix_H
