...
int main(void)
{
	...
	cout << "Enter the first (left) matrix:" << endl;

	LinBox::sparsemat_base<Field, Row> 
		B1(*LinBox::newSparsemat<Field, Row>(F));

	size_t m1, n1;
	m1 = B1.get_rowdim();
	n1 = B1.get_coldim();

	cout << "The first sparesemat matrix contains the following elements:" 
		<< endl << B1;

	cout << "Enter the second (right) matrix:" << endl;

	LinBox::sparsemat_base<Field, Row> 
		B2(*LinBox::newSparsemat<Field, Row>(F));

	size_t m2, n2;
	m2 = B2.get_rowdim();
	n2 = B2.get_coldim();

	cout << "The second sparesemat matrix contains the following elements:" 
		<< endl << B2;

	cout << "Enter a vector to be multiplied by the compose matrix." << endl
	...
	LinBox::sparsemat<Field, Row, Vector1> S11(B1), S12(B2);
	LinBox::Compose<Vector1> C1(&S11, &S12);
	LinBox::Blackbox_archetype<Vector1>& A1 = C1;
	...
	LinBox::sparsemat<Field, Row, Vector2> S21(B1), S22(B2);
	LinBox::Compose<Vector2> C2(&S21, &S22);
	LinBox::Blackbox_archetype<Vector2>& A2 = C2;
	...
	LinBox::sparsemat<Field, Row, Vector3> S31(B1), S32(B2);
	LinBox::Compose<Vector3> C3(&S31, &S32);
	LinBox::Blackbox_archetype<Vector3>& A3 = C3;
