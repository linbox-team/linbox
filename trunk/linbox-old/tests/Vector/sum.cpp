/* File:	tests/BlackBox/sum.h
 * Author:	William J. Turner for the LinBox group
 *
 * Run tests on vector utility sum
 */
...
int main(void)
{
	...
	LinBox::sum(F, z1, x1, y1);

	cout << "Using sum(F, z1, x1, y1) gives vector z1:" << endl;
	i = 0;
	...
	z1 = Vector1(n, zero);
	z1 = LinBox::sum(F, x1, y1);

	cout << "Using z1 = sum(F, x1, y1) gives vector z1:" << endl;
	i = 0;
	for (z1_iter = z1.begin(); z1_iter != z1.end(); z1_iter++, i++)
	{
		cout << "\tz1[" << i << "] = ";
	...
	z1 = y1;
	z1 = LinBox::sumin(F, z1, x1);

	cout << "Using z1 = y1; sumin(F, z1, x1) gives vector z1:" << endl;
	i = 0;
	for (z1_iter = z1.begin(); z1_iter != z1.end(); z1_iter++, i++)
	{
		cout << "\tz1[" << i << "] = ";
	...
	LinBox::sum(F, z2, x2, y2);

	cout << "Using sum(F, z2, x2, y2) gives vector z2:" << endl;
	...
	z2 = Vector2();
	z2 = LinBox::sum(F, x2, y2);

	cout << "Using z2 = sum(F, x2, y2) gives vector z2:" << endl;
	for (z2_iter = z2.begin(); z2_iter != z2.end(); z2_iter++)
	{
		cout << "\tz2[" << z2_iter->first << "] = ";
		F.write(cout, z2_iter->second); 
	...
	z2 = y2;
	z2 = LinBox::sumin(F, z2, x2);

	cout << "Using z2 = y2; sumin(F, z2, x2) gives vector z2:" << endl;
	for (z2_iter = z2.begin(); z2_iter != z2.end(); z2_iter++)
	{
		cout << "\tz2[" << z2_iter->first << "] = ";
		F.write(cout, z2_iter->second); 
	...
	LinBox::sum(F, z3, x3, y3);

	cout << "Using sum(F, z3, x3, y3) gives vector z3:" << endl;
	...
	z3 = Vector3();
	z3 = LinBox::sum(F, x3, y3);

	cout << "Using z3 = sum(F, x3, y3) gives vector z3:" << endl;
	for (z3_iter = z3.begin(); z3_iter != z3.end(); z3_iter++)
	{
		cout << "\tz3[" << z3_iter->first << "] = ";
		F.write(cout, z3_iter->second); 
	...
	z3 = y3;
	z3 = LinBox::sumin(F, z3, x3);

	cout << "Using z3 = y3; sumin(F, z3, x3) gives vector z1:" << endl;
	i = 0;
	for (z3_iter = z3.begin(); z3_iter != z3.end(); z3_iter++)
	{
		cout << "\tz3[" << z3_iter->first << "] = ";
