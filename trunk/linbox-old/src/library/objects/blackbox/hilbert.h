...
namespace LinBox
{
	...
	template <class Field, class Vector>
	inline Vector &hilbert<Field, Vector, vector_categories::sparse_sequence_vector_tag>
		::apply (Vector &y, const Vector& x) const
	{
		// Create zero vector to hold output
		y.clear ();
		...
		element zero = _F.zero (), entry = _F.zero (), temp = _F.zero ();
	...
	template <class Field, class Vector>
	inline Vector& hilbert<Field, Vector, vector_categories::sparse_associative_vector_tag>
		::apply (Vector &y, const Vector& x) const
	{
		y.clear ();
