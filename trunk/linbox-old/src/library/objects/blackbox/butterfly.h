...
template <class Vector, class Switch>
inline LinBox::butterfly<Vector, Switch>
::butterfly(size_t n, const Switch& S)
: _n(n), _switch(S)
{
...
template <class Vector, class Switch>
inline Vector& LinBox::butterfly<Vector, Switch>
...
} // Vector& butterfly<Vector, Switch>::applyin(const Vector& x) const

template <class Vector, class Switch>
inline Vector& 
LinBox::butterfly<Vector, Switch>
::applyTranspose(Vector& y, const Vector& x) const
{
	...
	{
#ifdef TRACE
		clog << "  Switching x[" << iter->first << "] and x["
			<< iter->second << "]: ";
#endif // TRACE
...
inline Vector& 
LinBox::butterfly<Vector, Switch>
::applyTransposein(Vector& x) const
{
#ifdef TRACE
	clog << "Called butterfly.applyTranspose(x)" << endl;
#endif // TRACE
		...
		)
{
	...
	if (n == n_p)
	{
		...
		log << "n = " << n << " is a power of two.  "
			<< "Resetting n_p to be half of n: n_p = " 
			<< n_p << endl;
#endif // TRACE
	} // if (n = n_p)
...
size_t LinBox::count_butterfly(size_t N, ostream& log = clog)
{
	// break inputs into groups of size powers of 2.
	// calculate size of groups, and powers of 2 that give sizes
	// store these values in vectors n and l, respectively
