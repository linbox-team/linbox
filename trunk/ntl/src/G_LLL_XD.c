...
GivensCache_XD::GivensCache_XD(long m, long n)
{
   ...
   long i;
   buf = new xdoubleptr[sz]; 
   if (!buf) Error("out of memory");
   for (i = 0; i < sz; i++)
      if (!(buf[i] = new xdouble[n+1])) Error("out of memory");
