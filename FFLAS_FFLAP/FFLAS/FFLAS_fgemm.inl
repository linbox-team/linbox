...
//                FFLAS_gemm: matrix multiplication routines   
//----------------------------------------------------------------------
...
inline  void FFLAS::WinoCalc( const Field& F, 
	...
	for (int i=0;i<mr;++i,d11+=lda,d11c+=ldc,d12c+=ldc, d21+=lda,dx2+=kr){
		for (int j=0;j<kr;++j)
	...
	for (int i=0;i<mr;++i, d21+=lda, d22+=lda, dx2+=kr){
		for (int j=0;j<kr;++j)
	...
	for (int i=0;i<kr;++i, d11+=ldb, d12+=ldb,dx3+=nr){
		for (int j=0;j<nr;++j)
	...
	for (int i=0;i<kr;++i,d22+=ldb,dx3+=nr){
		for (int j=0;j<nr;++j)
	...
	// S2 = S1 - A11 in X2
	d11 = A; dx2 = X2;
	for (int i=0;i<mr;++i,d11+=lda,dx2+=kr){
		for (int j=0;j<kr;++j)
			F.subin(*(dx2+j),*(d11 + j));
	}
	...
	// T4 = T2 - B21 in X3
	d21 = B+kr*ldb;dx3=X3;
	for (int i=0;i<kr;++i,d21+=ldb,dx3+=nr){
		for (int j=0;j<nr;++j)
			F.subin(*(dx3+j),*( d21 + j));
	}
	...
	for (int i=0;i<mr;++i, d12+=lda, dx2+=kr,d12c+=ldc,dx1+=nr,d21c+=ldc,d22c+=ldc){
		for (int j=0;j<kr;++j)
	...
	// U5 = U4 + P3 in C12
	d12c = C+nr;dx1=X1;
	for (int i=0;i<mr;++i, d12c+=ldc, dx1+=nr){
		for (int j=0;j<nr;++j)
	...
	for (int i=0;i<mr;++i,d21c+=ldc,d22c+=ldc){
		for (int j=0;j<nr;++j)
	...
	for (int i=0;i<mr;++i, d21c+=ldc, dx1+=nr){
		for (int j=0;j<nr;++j)
...
//-------------------------------------------------------------------
template<>
inline  DoubleDomain::element* FFLAS::fgemm( DoubleDomain& D,
			      const enum FFLAS_TRANSPOSE ta,
			      const enum FFLAS_TRANSPOSE tb,
			      const size_t m, const size_t n, const size_t k,
...
FFLAS::fgemm( Field& F,
	...
	if ( !F.iszero(beta) || !F.isone(alpha) ||
		...
		elt* t_X3[winostep+1];
		// Temporary space allocation
		for (int i=winostep;(i>=1)&&(kr>kmax);--i){
