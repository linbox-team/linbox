...
namespace LinBox
{
	...
	BasicReader::BasicReader(const Writer &W) :
		XMLTree()
	{
		errorString = new std::string;
		parseErrorLine = new int;
	...
	//
	BasicReader::BasicReader(std::istream &dataStream, const char* encoding) :
		XMLTree()
	{
		errorString = new std::string();
		parseErrorLine = new int;
	...
	//
	BasicReader::BasicReader(const BasicReader &Bin) :
		XMLTree()
	{
...
Reader::Reader(std::istream &in, const char* encoding) :
	BasicReader(in, encoding)
{}

Reader::Reader(const Reader &Rin) :
	BasicReader(Rin)
...
ReaderIterator::ReaderIterator(const ReaderIterator &x) :
	Reader(x)
{ // use polymorphism, call the base copy constr.
	calledOnText = x.calledOnText;
	eqIter = x.eqIter;
}

// The "real" constructor used by the Reader class
ReaderIterator::ReaderIterator(const Reader &x, std::list<node*>::iterator it, bool onText) :
	Reader(x)
