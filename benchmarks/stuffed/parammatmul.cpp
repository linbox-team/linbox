...
{
	uint64_t x = 1u << logo2(p-1);
	return logo2(x*(p-1)) - logo2(p-1);
}
...
double trial(unsigned int seed)
{
	...
	for (volatile int t = 0; t < TRIALS; ++t) {
		for (unsigned int i = 0; i < AXPY_COUNT; ++i) {
			for (unsigned int j = 0; j < Field::Unit::get_r(); ++j) {
			cmd.axpyin(B,A,1u << j);
			}
			if ((i % f.m4rm_freq() == 0 && i != 0) or (i == AXPY_COUNT-1)) {
				cmd.normalize(B);
			}
		}
	}
	...
	std::cout << " " << Field::Unit::get_r() << " " << Field::Unit::get_l() << " ";// << ops;
	return ops;
}
