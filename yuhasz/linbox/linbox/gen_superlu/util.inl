...
#include "sp_defs.h"
#include "util.h"
#include "memory.h" // A.Duran 11/8/02
/* 
...
void
Destroy_SuperMatrix_Store(SuperMatrix<Field> *A)
{
...
template<class Field>
void
Destroy_CompCol_Matrix(SuperMatrix<Field> *A)
{
    SUPERLU_FREE( ((NCformat<Field> *)A->Store)->rowind );
    SUPERLU_FREE( ((NCformat<Field> *)A->Store)->colptr );
    SUPERLU_FREE( ((NCformat<Field> *)A->Store)->nzval );
    SUPERLU_FREE( A->Store );
}

template<class Field>
void
Destroy_SuperNode_Matrix(SuperMatrix<Field> *A)
{
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->rowind );
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->rowind_colptr );
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->nzval );
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->nzval_colptr );
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->col_to_sup );
    SUPERLU_FREE ( ((SCformat<Field> *)A->Store)->sup_to_col );
    SUPERLU_FREE ( A->Store );
}
...
template<class Field>
void
Destroy_CompCol_Permuted(SuperMatrix<Field> *A)
{
    SUPERLU_FREE ( ((NCPformat<Field> *)A->Store)->colbeg );
    SUPERLU_FREE ( ((NCPformat<Field> *)A->Store)->colend );
    SUPERLU_FREE ( A->Store );
}
...
template<class Field>
void
Destroy_Dense_Matrix(SuperMatrix<Field> *A)
{
    DNformat<Field>* Astore = (DNformat<Field>*)A->Store;
...
template<class Field>
void
countnz(const int n, int *xprune, int *nnzL, int *nnzU, GlobalLU_t<Field> *Glu)
{
...
template<class Field>
void
fixupL(const int n, const int *perm_r, GlobalLU_t<Field> *Glu)
{
...
void super_stats(int nsuper, int *xsup)
{
    ...
    for (i = 0; i <= nsuper; i++) {
        isize = xsup[i+1] - xsup[i];
        whichb = static_cast<int>((float) isize / max_sup_size * NBUCKS);
    ...
    for (i = 0; i < NBUCKS; i++) {
        bl = static_cast<int>((float) i * max_sup_size / NBUCKS); /* A. Duran */
        bh = static_cast<int>((float) (i+1) * max_sup_size / NBUCKS); /* A. Duran */
        printf("\tsnode: %d-%d\t\t%d\n", bl+1, bh, bucket[i]);
    }
