...
struct MyIntegerModularCharpoly {
	...
	MyIntegerModularCharpoly(const Blackbox& b, const MyMethod& n, const std::vector<typename Blackbox::Field::Element>& ve,
					const std::vector<typename Blackbox::Field::Element >& p	)
	: A(b), M(n), vD(ve), mul(p) {}

	MyIntegerModularCharpoly(MyIntegerModularCharpoly& C) : MyIntegerModularCharpoly(C.A,C.M,C.vD,C.mul){}
